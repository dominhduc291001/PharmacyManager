import { Component, ViewEncapsulation, OnInit, Input, AfterViewInit } from '@angular/core';
...
@Component({
    selector: 'athrz-detail-regist',
    templateUrl: './athrz-detail-regist.component.html',
    styleUrls: ['../test-share.component.css'],
    encapsulation: ViewEncapsulation.None
})

export class AthrzDetailRegistComponent implements OnInit {
    ...
    constructor(private _auth: AuthService, private message: NzMessageService,
    ...
    changeConfmSn(event: any): void {
        ...
        if (event.value !== null) {
            ...
            this._inspection.copyFormSent2014(result).subscribe((data) => {
            ...
            result.athrzSn = this.dataTemp.inspecTemp.athrzSn;
            result.athrzDetailSn = this.dataTemp.inspecTemp.athrzDetailSn;
            this._inspection.copyFormSent2014Ath(result).subscribe((data) => {
                this.dataDetail.athrzSn = data.athrzSn;
                this.dataDetail.mrnrSeCode = data.mrnrSeCode;
                this.dataDetail.athrzDetailSn = data.athrzDetailSn;
                // eslint-disable-next-line prefer-const
        ...
        if (this.dataDetail.mrnrSeCode === '') {
            ...
            if (this.meterPopupKey !== 16 && this.checkMrnrCode(this.dataDetail.mrnrSeCode)) {
                if (this.dataDetail.athrzSn > 0 && this.dataDetail.athrzDetailSn > 0) {
                    ...
                    this.tempSave.userId = this._auth.userOnl;
                    this._inspection.tempSave(this.tempSave).subscribe(async (data) => {
                        this.dataDetail.athrzSn = data.athrzSn;
                        this.dataDetail.athrzDetailSn = data.athrzDetailSn;
                        this.tempSave.inspecTemp.athrzDetailSn = data.athrzDetailSn;
    ...
    mrnrStdPopup(): void {
        ...
        switch (this.meterPopupKey) {
            ...
            case 3:
                ...
                break;
            case 4:
                const drawerMeter4 = this.drawerService.create<IptWeightPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격및형식(분동)',
                        nzContent: IptWeightPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter4.open();
                drawerMeter4.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 5:
                const drawerMeter5 = this.drawerService.create<IptGasmeterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(가스미터)',
                        nzContent: IptGasmeterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter5.open();
                drawerMeter5.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 6:
                const drawerMeter6 = this.drawerService.create<IptWaterMeterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(수도미터)',
                        nzContent: IptWaterMeterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter6.open();
                drawerMeter6.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 7:
                const drawerMeter7 = this.drawerService.create<IptHotWaterMeterComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(온수미터)',
                        nzContent: IptHotWaterMeterComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter7.open();
                drawerMeter7.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 8:
                const drawerMeter8 = this.drawerService.create<IptOilMeterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격및형식(오일미터)',
                        nzContent: IptOilMeterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter8.open();
                drawerMeter8.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
            ...
            case 9:
                ...
                break;
            case 10:
                const drawerMeter10 = this.drawerService.create<IptLgpDispenserComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격및형식(LPG미터)',
                        nzContent: IptLgpDispenserComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter10.open();
                drawerMeter10.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 11:
                const drawerMeter11 = this.drawerService.create<IptGrarulerTanvoComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(눈새김 탱크)',
                        nzContent: IptGrarulerTanvoComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter11.open();
                drawerMeter11.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 12:
                const drawerMeter12 = this.drawerService.create<IptHeatMeterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(적산열량계)',
                        nzContent: IptHeatMeterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter12.open();
                drawerMeter12.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 13:
                const drawerMeter13 = this.drawerService.create<IptElecmeterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(전력량계)',
                        nzContent: IptElecmeterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter13.open();
                drawerMeter13.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
                ...
                break;
            case 14:
                const drawerMeter14 = this.drawerService.create<IptElementsWaterPopupComponent,
                    { dataMain: AthrzDetailRegist }>({
                        nzTitle: '규격 및 형식(요소수 미터)',
                        nzContent: IptElementsWaterPopupComponent,
                        nzWidth: this.sizeDrawer,
                        nzContentParams: {
                            dataMain: this.dataDetail
                        }
                ...
                drawerMeter14.open();
                drawerMeter14.afterClose.subscribe((data) => {
                    if (data.length !== 0) {
                        this.dataDetail.selMrnrStdPopup = [];
                    }
                    for (const item of data) {
    ...
    async detailReg(): Promise<any> {
        ...
        if (this.isLive === false) {
            for (const item of this.dataView.entrprsBplcAdresList) {
            ...
            for (const item of this.dataView.entrprsFctryAdresList) {
                if (this.dataDetail.athrzAreaAdres === item.entrprsAdresSn) {
                    this.tempSave.inspecTemp.athrzAreaAdresSeCode = item.entrprsAdresSeCode;
                    this.tempSave.inspecTemp.athrzAreaZip = item.zip;
                    this.tempSave.inspecTemp.athrzAreaBassAdres = item.bassAdres;
            ...
            for (const item of this.dataView.entrprsMeterAdresList) {
                if (this.dataDetail.athrzAreaAdres === item.entrprsAdresSn) {
                    this.tempSave.inspecTemp.athrzAreaAdresSeCode = item.entrprsAdresSeCode;
                    this.tempSave.inspecTemp.athrzAreaZip = item.zip;
                    this.tempSave.inspecTemp.athrzAreaBassAdres = item.bassAdres;
    ...
    getSelStd(): Promise<any> {
        ...
        result.fomConfmSn = this.dataDetail.fomConfmSn;
        result.gubun = 1;
        return this._inspection.copyFormSent2014(result).toPromise();
    }
    ...
    getSelStdAth(): Promise<any> {
        ...
        result.mrnrSeCode = this.dataDetail.mrnrSeCode;
        result.gubun = 1;
        return this._inspection.copyFormSent2014Ath(result).toPromise();
    }

    deleteAthrzDetail(athrzSn: number, athrzDetailSn: number): void {
        if (athrzSn !== null && athrzDetailSn !== null) {
